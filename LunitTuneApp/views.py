...
#     """Абсолютный URL для File/ImageField, либо пустая строка."""
#     try:
#         u = getattr(f, "url", "")
...
#     Формат под фронт:
#       - time: число (секунды)
#       - audio / cover: абсолютные URL
#     Дополнительно кладём audio_url/cover_url/stream_url и time_label для совместимости.
#     """
#     audio_abs = _abs_url(request, t.audio)
#     cover_abs = _abs_url(request, t.cover)
#     base = {
#         "id": t.id,
#         "name": t.name,
...
#         "adult": bool(t.adult),
#         "time": int(t.time or 0),            # как число
#         "audio": audio_abs,                  # ожидаемые ключи фронтом
#         "cover": cover_abs,
...
#         "audio_url": audio_abs,
#         "cover_url": cover_abs,
#         "stream_url": request.build_absolute_uri(reverse('stream_track', args=[t.id])) if audio_abs else "",
#         "time_label": _mmss_from_seconds(t.time),
#     })
#     return base


# def playlist_json(p: Playlist, request) -> dict:
#     return {
#         "id": str(p.id),
#         "title": p.title,
#         "description": p.description or "",
#         "cover": _abs_url(request, p.cover),        # фронт ждёт именно cover
#         "owner_id": (str(getattr(p, "owner_id", "")) if getattr(p, "owner_id", None) else ""),
...
#         "id": a.id,
#         "title": a.title,
#         "name": a.title,                # унификация под фронт
#         "author": a.author or "",
#         "author_fk_id": str(getattr(a, "author_fk_id", "")),
#         "genreid": a.genreid or "",
#         "playsnum": int(a.playsnum or 0),
#         "adult": bool(a.adult),
#         "duration_seconds": dur,
#         "time": dur,                    # фронт ждёт как у треков (число)
#         "time_label": _mmss_from_seconds(dur),
#         "info": a.info or "",
#         "chapter": int(a.chapter or 0),
#         "audio_file": audio_abs,
#         "cover_image": cover_abs,
#         "audio": audio_abs,             # ключи как у Track
#         "cover": cover_abs,
#         "created_at": _iso(created),
#         "update_at": _iso(updated),
#     }

# def podcast_json(p, request):
...
#         "id": p.id,
#         "title": p.title,
#         "name": p.title,                # унификация
#         "episode": int(p.episode or 0),
#         "host": p.host or "",
#         "host_fk_id": str(getattr(p, "host_fk_id", "")),
...
#     qs = AudioBook.objects.all().order_by("-id")
#     if q:
#         qs = qs.filter(
...
#     paged = _paginate_qs(qs, request)

#     def ab_row(a: AudioBook):
#         return {
#             "id": a.id,
#             "title": a.title,
#             "author": a.author or "",
...
#             "info": a.info or "",
#             "chapter": int(a.chapter or 0),
#             "audio_file": _abs_url(request, a.audio_file),
#             "cover_image": _abs_url(request, a.cover_image),
#             "created_at": _iso(getattr(a, "created_at", None)) or _iso(datetime.now(timezone.utc)),
#             "update_at": _iso(getattr(a, "update_at", None)) or _iso(getattr(a, "updated_at", None) or datetime.now(timezone.utc)),
#         }

#     data = [ab_row(x) for x in paged["items"]]
#     return JsonResponse({
#         "success": True,
#         "items": data,
#         "page": paged["page"],
...
#     """
#     Либо обычный список треков, либо (совместимость с фронтом):
#     если ?albumid=... совпадает с плейлистом (id или title),
#     вернуть треки из этого плейлиста.
#     """
#     try:
#         --- Совместимость: albumid как идентификатор ПЛЕЙЛИСТА ---
#         albumid = (request.GET.get('albumid') or '').strip()
#         if albumid:
#             from .models import Playlist, PlaylistItem  # локальный импорт во избежание циклов

#             pl = None
#             если число — ищем по PK
#             if albumid.isdigit():
#                 pl = Playlist.objects.filter(pk=int(albumid)).first()
#             если не нашли по PK — пробуем по title без учета регистра
#             if pl is None:
#                 pl = Playlist.objects.filter(title__iexact=albumid).first()
...
#                 for it in qs_items:
#                     if it.track:
#                         data.append(track_json(it.track, request))
#                 return JsonResponse({
#                     'success': True,
#                     'tracks': data,
#                     'page': 1,
...
#         qs = qs.order_by('-created_at' if hasattr(Track, 'created_at') else '-id')
...
#                     if v is not None:
#                         setattr(new, f, v or None)
#                 if tf: new.audio = tf
#                 if cf: new.cover = cf
#                 new.save()

#                 пересчитать длительность, если новый файл
...
#             пересчитать длительность, если пришёл новый аудиофайл
...
#     """
#     Список треков по полю (artistid/albumid/genreid) с пагинацией.
#     """
...
#     qs = Track.objects.all().exclude(**{f"{field}__isnull": True}).exclude(**{f"{field}": ""})
#     if value:
...
#     qs = qs.order_by(field, "-created_at" if hasattr(Track, "created_at") else "-id", "name")
...
#         if role == 'admin' and hasattr(User, 'is_admin'):
#             qs = qs.filter(is_admin=True)
#         elif role == 'staff' and hasattr(User, 'is_staff'):
#             if hasattr(User, 'is_admin'):
#                 qs = qs.filter(is_staff=True, is_admin=False)
#             else:
#                 qs = qs.filter(is_staff=True)
#         elif role == 'client' and hasattr(User, 'is_client'):
...
#         data = []
#         for u in items:
#             full_name = getattr(u, 'full_name', '') or (u.email.split('@')[0] if getattr(u, 'email', '') else '')
#             role_out = 'admin' if getattr(u, 'is_admin', False) else ('staff' if getattr(u, 'is_staff', False) else 'client')
#             data.append({
#                 'id': u.id,
#                 'email': u.email,
#                 'full_name': full_name,
...
#                 'is_client': getattr(u, 'is_client', False),
#                 'role': role_out,
#             })
...
#         role = (request.POST.get('role') or 'client').lower()
...
#         if role == 'admin':
#             u.is_admin = True; u.is_staff = True; u.is_client = False
#         elif role == 'staff':
#             u.is_admin = False; u.is_staff = True; u.is_client = False
#         else:
#             u.is_admin = False; u.is_staff = False; u.is_client = True

#         u.set_password(password)
#         u.save()
#         return JsonResponse({'success': True, 'id': u.id}, status=200)
#     except Exception as e:
#         return JsonResponse({'success': False, 'error': str(e)}, status=500)
...
#         return JsonResponse({'success': True, 'deleted': deleted}, status=200)
#     except Exception as e:
#         return JsonResponse({'success': False, 'error': str(e)}, status=500)
...
# from django.db.models import Count as _Count  # чтоб не конфликтовало с верхним импортом

# @require_http_methods(["GET"])
# def list_playlists(request):
#     """
#     Возвращает items в формате playlist_json(...) + tracks_count.
#     cover -> абсолютный URL (ключ 'cover', как ждёт фронт).
#     """
#     try:
#         qs = Playlist.objects.annotate(tracks_count=_Count('items')).order_by('-created_at', 'id')
...
#             obj["tracks_count"] = int(getattr(p, "tracks_count", 0))
#             data.append(obj)
...
#         ---------- РЕЗОЛВИМ ИДЕНТИФИКАТОРЫ В РЕАЛЬНЫЕ Track.id ----------
#         incoming = []
...
#                         "type": (str(item.get("type") or item.get("kind") or "").strip().upper() or None)
#                     })
#                 else:
#                     incoming.append({"id": str(item).strip(), "type": None})

#         def _split_kind(rid: str, forced: str | None):
#             s = rid.strip()
#             if s.startswith("__ab_"):     return "AB", s[5:]
#             if s.startswith("__pc_"):     return "PC", s[5:]
#             if forced:                    return forced, s
#             up = s.upper()
#             TR|AB|PC[:\-#_]<body>
#             for prefix in ("TR", "AB", "PC"):
#                 if up.startswith(prefix):
#                     body = s[len(prefix):].lstrip(":-#_")
#                     return prefix, (body or s)
#             return None, s

#         создаём shadow-треки для AB/PC при необходимости
#         real_track_ids: list[str] = []
#         for rec in incoming:
#             raw_id = rec["id"]
#             forced = rec["type"]
#             kind, body = _split_kind(raw_id, forced)

#             1) Явный/обычный трек
#             if kind in (None, "TR"):
#                 t = Track.objects.filter(pk=body).first()
#                 if t:
#                     real_track_ids.append(t.pk)
#                     continue
#                 иногда фронт шлёт число, которое совпадает с AB/PC
#                 if body.isdigit():
#                     ab = AudioBook.objects.filter(pk=int(body)).first()
#                     if ab:
#                         sid = f"__ab_{ab.pk}"
...
#                             t = Track(
#                                 id=sid,
#                                 name=ab.title or f"Audiobook #{ab.pk}",
#                                 artistid=(ab.author or "")[:100],
...
#                             )
#                             if ab.audio_file: t.audio = ab.audio_file
#                             if ab.cover_image: t.cover = ab.cover_image
#                             t.save()
#                         real_track_ids.append(t.pk)
#                         continue
#                     pc = PodcastEpisode.objects.filter(pk=int(body)).first()
#                     if pc:
#                         sid = f"__pc_{pc.pk}"
#                         t = Track.objects.filter(pk=sid).first()
#                         if not t:
#                             t = Track(
#                                 id=sid,
#                                 name=pc.title or f"Podcast #{pc.pk}",
#                                 artistid=(pc.host or "")[:100],
...
#                             )
#                             if pc.audio_file: t.audio = pc.audio_file
#                             if pc.cover_image: t.cover = pc.cover_image
#                             t.save()
#                         real_track_ids.append(t.pk)
#                         continue
#                 неизвестно — пропускаем
#                 continue

#             2) Аудиокнига → shadow Track
#             if kind == "AB" and body.isdigit():
#                 ab = AudioBook.objects.filter(pk=int(body)).first()
#                 if ab:
#                     sid = f"__ab_{ab.pk}"
#                     t = Track.objects.filter(pk=sid).first()
...
#             3) Подкаст → shadow Track
#             if kind == "PC" and body.isdigit():
#                 pc = PodcastEpisode.objects.filter(pk=int(body)).first()
#                 if pc:
#                     sid = f"__pc_{pc.pk}"
#                     t = Track.objects.filter(pk=sid).first()
...
#         with transaction.atomic():
#             pl = Playlist.objects.create(title=title, description=description, owner=request.user)
#             if (f := request.FILES.get('cover')):
#                 pl.cover = f
#                 pl.save()

#             tracks = list(Track.objects.filter(pk__in=ordered_ids))
#             tmap = {t.pk: t for t in tracks}
#             bulk = []
#             pos = 0
...
#         payload["tracks_count"] = len(ordered_ids)
#         return JsonResponse({'success': True, 'playlist': payload}, status=200)
...
#                     t = Track.objects.filter(pk=body).first()
#                     if t:
#                         real_track_ids.append(t.pk); continue
#                     if body.isdigit():
#                         ab = AudioBook.objects.filter(pk=int(body)).first()
#                         if ab:
#                             sid = f"__ab_{ab.pk}"
...
#     try:
#         pl = Playlist.objects.get(pk=pl_id, owner=request.user) if hasattr(Playlist,'owner') else Playlist.objects.get(pk=pl_id)
#         pl.delete()
#         return JsonResponse({'success': True})
#     except Playlist.DoesNotExist:
#         return JsonResponse({'success': False, 'error': 'Playlist not found'}, status=404)
...
#     1) ищем по PK, затем fallback по title (как было)
#     pl = Playlist.objects.filter(pk=pl_id).first()
#     if not pl:
#         pl = Playlist.objects.filter(title__iexact=pl_id).first()
#     if not pl:
#         return JsonResponse({"success": False, "error": "Playlist not found"}, status=404)

#     2) собираем треки в порядке position
#     items_qs = (PlaylistItem.objects
#                 .filter(playlist=pl)
#                 .select_related("track")
#                 .order_by("position", "id"))
...
#     for it in items_qs:
#         if it.track:
#             tracks.append(track_json(it.track, request))   # строго формат Track из твоего TS

#     3) payload + счетчик треков
#     payload = {
#         "id": str(pl.id),
#         "title": pl.title or "",
#         "description": pl.description or "",
#         "cover": _abs_url(request, getattr(pl, "cover", None)),
#         "owner_id": str(getattr(pl, "owner_id", "") or ""),
#         "created_at": _iso(getattr(pl, "created_at", None)) or "",
#         "tracks": tracks,                    # <-- вот это ждёт фронт
...
# @require_http_methods(["GET"])
# def playlist_items(request):
#     """
#     Возвращает элементы плейлиста в формате:
#     { id: string, playlist_id: string, track: string, position: string }

#     Принимает:
#       - ?playlist_id=<id|title>
#       - опционально ?full=1 — добавит track_obj (можно игнорить на фронте).
#     """
#     raw = (request.GET.get("playlist_id") or request.GET.get("playlistId") or "").strip()
#     if not raw:
#         return JsonResponse({"items": []}, status=200)

#     Найти плейлист: если число — по PK, иначе — по title (без учёта регистра)
#     pl = None
#     if raw.isdigit():
...
#     full = (request.GET.get("full") or "").lower() in ("1", "true", "yes")
...
#             "position": str(it.position),
#         }
#         if full and it.track:
#             row["track_obj"] = track_json(it.track, request)  # фронт может игнорировать
#         items.append(row)
...
# =========================
# Pagination helpers (generic)
# =========================

# def _paginate_qs(qs, request, default_size=10):
#     try:
...
#     p = Paginator(qs, page_size)
#     total_pages = max(1, p.num_pages or 1)
#     page = max(1, min(page, total_pages))
#     items = p.page(page).object_list

#     return {"items": items, "page": page, "page_size": page_size, "total": p.count, "total_pages": total_pages}


# def _paginate(qs, request, default_page_size=10):
#     page = int(request.GET.get("page", 1) or 1)
#     page_size = int(request.GET.get("page_size", default_page_size) or default_page_size)
#     paginator = Paginator(qs, page_size)
#     try:
#         page_obj = paginator.page(page)
#     except EmptyPage:
...
#     q = (request.GET.get("q") or "").strip()
#     qs = PodcastEpisode.objects.all().order_by("-id")
#     if q:
#         qs = qs.filter(
#             Q(title__icontains=q) |
#             Q(host__icontains=q) |
...
#             "playsnum": int(p.playsnum or 0),
#             "adult": bool(p.adult),
#             "duration_seconds": int(p.duration_seconds or 0),
#             "info": p.info or "",
#             "audio_file": _abs_url(request, p.audio_file),
#             "cover_image": _abs_url(request, p.cover_image),
#             "created_at": _iso(getattr(p, "created_at", None)) or _iso(datetime.now(timezone.utc)),
#             "update_at": _iso(getattr(p, "update_at", None)) or _iso(getattr(p, "updated_at", None) or datetime.now(timezone.utc)),
#         }

#     data = [pc_row(x) for x in paged["items"]]
#     return JsonResponse({
#         "success": True,
#         "items": data,
#         "page": paged["page"],
...
#         )
#     paged = _paginate(qs, request, default_page_size=10)
